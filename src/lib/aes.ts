...
function initialize(): void {
    ...
    */
    ...
    const xtime = new Array<number>(256);
    for (let i = 0; i < 128; ++i) {
        xtime[i] = i << 1;
 ...
 */
function _expandKey(key: number[]): number[] {
 ...
 */
export function updateBlock(w: number[], input: number[], output: number[]): void {
    /*
    ...
    */
    ...
    for (let round = 1; round < Nr; ++round) {
        ...
        const a2 = m0[a >>> 24] ^ m1[(b >>> 16) & 255] ^ m2[(c >>> 8) & 255] ^ m3[d & 255] ^ w[++i];
        const b2 = m0[b >>> 24] ^ m1[(c >>> 16) & 255] ^ m2[(d >>> 8) & 255] ^ m3[a & 255] ^ w[++i];
        const c2 = m0[c >>> 24] ^ m1[(d >>> 16) & 255] ^ m2[(a >>> 8) & 255] ^ m3[b & 255] ^ w[++i];
